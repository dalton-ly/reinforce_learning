...
// 迭代解法
VectorXd iterativeSolution(const MatrixXd &P, const VectorXd &r, int maxIterations = 1000,
...
VectorXd solveBOE(Actions acts, const vector<vector<StateType>> &grid, double GAMMA, int NUM_STATES,
...
int main() {
    ...
    cout << "solve in : 5x5 Grid world, gamma: 0.9,   rboundary = rforbidden = 0, rtarget = 2, "
         ...
         << endl
         << endl;
    ans = solveBOE(acts, grid, 0.9, numStates, config{0, 0, 2, 1});
    m   = Eigen::Map<Eigen::MatrixXd>(ans.data(), 5, 5);
    cout << m.transpose() << endl << endl;
